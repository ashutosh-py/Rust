...
impl<'tcx> MutVisitor<'tcx> for FixReturnPendingVisitor<'tcx> {
    ...
    fn visit_assign(
        ...
        if let Rvalue::Aggregate(ref mut kind, _) = rvalue {
...
fn gather_dropline_blocks<'tcx>(body: &mut Body<'tcx>) -> BitSet<BasicBlock> {
    let mut dropline: BitSet<BasicBlock> = BitSet::new_empty(body.basic_blocks.len());
