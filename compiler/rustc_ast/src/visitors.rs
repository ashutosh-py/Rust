    ...
    ($trait: ident $(<$lt: lifetime>)? $(, $mut: ident)?) => {
        ...
        macro_rules! mutability_dependent {
            ...
            (mut) => {
                ...
                fn visit_meta_list_item(&mut self, list_item: ref_t!(NestedMetaItem)) -> result!() {
        ...
        derive_copy_clone!{
            ...
            pub enum FnKind<'a> {
                ...
                Fn(FnCtxt, Ident, &'a $($mut)? FnSig, &'a $($mut)? Visibility, &'a $($mut)? Generics, &'a $($mut)? Option<P<Block>>),
