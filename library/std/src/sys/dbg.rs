...
#[cfg(any(all(target_os = "macos", target_pointer_width = "64"), target_os = "freebsd"))]
mod os {
    ...
    // that defines `struct kproc_info` be of `648` bytes on the 64-bit system. That has
    // not changed since macOS 10.13 (released in 2017) at least, validated by building
    // a C program in XCode while changing the build target.
    #[cfg(all(target_os = "macos", target_pointer_width = "64"))]
    const KINFO_PROC_SIZE: usize = 648;
    #[cfg(all(target_os = "macos", target_pointer_width = "64"))]
    const KINFO_PROC_FLAGS_OFFSET: u64 = 32;
    ...
    #[cfg(all(target_os = "freebsd", target_pointer_width = "64"))]
    const KINFO_PROC_FLAGS_OFFSET: u64 = 368;

    #[cfg(all(target_os = "freebsd", target_pointer_width = "32"))]
    const KINFO_PROC_SIZE: usize = 768;
    #[cfg(all(target_os = "freebsd", target_pointer_width = "32"))]
    const KINFO_PROC_FLAGS_OFFSET: u64 = 296;
...
// 32-bit macOS Rust targets are no longer shipped.
#[cfg(not(any(
    target_os = "windows",
...
mod os {
    ...
    pub(super) fn is_debugger_present() -> Option<DebuggerPresence> {
        // Mark the enum variants as used in this particular case
        // where the functionality is not implemented.
        let _ = DebuggerPresence::Detected;
        let _ = DebuggerPresence::NotDetected;
